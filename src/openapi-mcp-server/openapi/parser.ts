...
export class OpenAPIToMCPConverter {
  ...
  convertOpenApiSchemaToJsonSchema(
    ...
    if (originalType as string === 'timestamp') {
        ...
        result.type = originalType as IJsonSchema['type']; // Use the standard type
        if (baseDescription) {
  ...
  private convertOperationToMCPMethod(operation: OpenAPIV3.OperationObject, method: string, path: string, baseName: string): NewToolMethod | null {
  ...
  private generateNameFromPath(method: string, path: string): string {
