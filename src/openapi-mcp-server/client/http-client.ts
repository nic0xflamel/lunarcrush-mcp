...
// console.log('[MCP Server Log] http-client.ts module loaded.');
...
export class HttpClient {
  ...
  private requestIntervalMs: number = 1000
  ...
  async executeOperation<T = any>(
    ...
    const paramsUsedInPathOrHeader: Set<string> = new Set(); // Keep track of params used in path or header
  ...
  private resolveRequestBody(body: OpenAPIV3.RequestBodyObject | OpenAPIV3.ReferenceObject): OpenAPIV3.RequestBodyObject | null {
      if (!('$ref' in body)) {
          ...
          const definition = (this.client as any).definition;
          if (!definition) {
          ...
          try {
              for (const part of parts) {
                  ...
                  if (current === undefined) throw new Error(`Part '${part}' not found in path.`);
              }
  ...
  private generateOperationIdFallback(method: string, path: string): string {
